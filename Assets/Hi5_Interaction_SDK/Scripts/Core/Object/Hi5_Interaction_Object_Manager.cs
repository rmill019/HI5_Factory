			...
			if (messageKey.CompareTo(Hi5_MessageKey.messageUnPinchObject) == 0)
			{
				...
				if ( mObjectDic.ContainsKey(objectId))
				{
                    ...
                    if (pinchObject != null && pinchObject.mObjectType == EObject_Type.ECommon)
                    {
						...
						if(OtherHandRelease)
						{
							Hi5_Interaction_Message.GetInstance().DispenseMessage(Hi5_MessageKey.messagePichOtherHandRealse, hand, objectId);
						}
                        ...
                        if (isRelease)
                        {
                            ...
                            {
                                pinchObject.CacullateThrowMove(hand.mPalm.transform,hand);
