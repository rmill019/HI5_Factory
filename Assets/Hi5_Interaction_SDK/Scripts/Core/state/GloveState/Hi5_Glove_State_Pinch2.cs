            ...
            if (fingerColliders.Count > 1)
            {
               // Debug.Log("fingerColliders.Count > 1" );
                ...
                foreach (KeyValuePair<Hi5_Glove_Interaction_Finger_Type, Hi5_Glove_Interaction_Finger> item in fingerColliders)
                {
                    ...
                    //}
                    //角度释放
                    if (fingerColliders[item.Key].IsPinch2Realease(Hi5_Interaction_Const.FingerPich2Release))
                    {
                        isRelease = true;
                        return;
                     }
                    
                    //Hi5_Glove_Interraction_Item objectTemp = Hi5_Interaction_Object_Manager.GetObjectManager().GetItemById(objectId);
                    //if (objectTemp != null)
                    //{
                    //    bool ContactIsSelf = false;
                    //    float dis = fingerColliders[item.Key].Pich2RealseTailDistance(objectTemp, out ContactIsSelf);
                    //    //if(item.Key == Hi5_Glove_Interaction_Finger_Type.EMiddle)
                    //    if (ContactIsSelf)
                    //        Debug.Log("item.Key"+ item.Key+"dis" + dis);
                    //    else
                    //        Debug.Log("ContactIsSelf item.Key" + item.Key+"dis" + dis);
                    //    if (ContactIsSelf)
                    //    {
                    //        if (item.Key == Hi5_Glove_Interaction_Finger_Type.EMiddle)
                    //        {
